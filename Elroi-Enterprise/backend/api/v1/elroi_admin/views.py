...
class UserGuideUploadApi(mixins.ListModelMixin, GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        self.queryset = UserGuideModel.objects.filter(elroi_id__exact=request.user.enterprise.elroi_id)
        return self.list(request, *args, **kwargs)

    def list(self, request, *args, **kwargs):
        response = super(UserGuideUploadApi, self).list(request, *args, **kwargs)
        uploads_obj = []
        for upload_file in self.queryset:
            files = ['/media/user_guide/' + str(guide_file.file) for guide_file in upload_file.uploads.filter(user_guide=upload_file.pk)]
        ...
        try:
            ...
            if request.FILES['file']:
                try:
                    ...
                    open(os.path.join(settings.UPLOAD_USER_GUIDE_FOLDER, file.name), 'wb').write(file.file.read())
